...
/* RWW EEPROM section, 1 KB on the E15 (samd21e15bu.h) — half the E16's.
 * Non-zero enables the bootloader's RWWEE write path, which is where the
 * app's runtime motion config lives — see firmware/DESIGN.md §7. Sized
 * per-part because the shared CMSIS instance header gets it wrong; see
 * inc/uf2.h. */
#define NVMCTRL_RWW_EEPROM_SIZE 0x400
